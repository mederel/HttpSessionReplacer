...
public class TestServletContextAndFilterTransformer {
  ...
  public void testTransformFilter() throws IllegalClassFormatException, IOException {
    ...
    ProtectionDomain protectionDomain = mock(ProtectionDomain.class);
    byte[] classfileBuffer = readFully(is);
    byte[] result = sct.transform(clazz.getClassLoader(), clazz.getName(), clazz, protectionDomain, classfileBuffer);
  ...
  public void testTransformInheritedServletContext() throws IllegalClassFormatException, IOException {
    ...
    InputStream is =
        clazz.getClassLoader().getResourceAsStream("com/amadeus/session/agent/MockSubServletContext.class");
    ProtectionDomain protectionDomain = mock(ProtectionDomain.class);
    byte[] classfileBuffer = readFully(is);
    byte[] result = sct.transform(clazz.getClassLoader(), clazz.getName(), clazz, protectionDomain, classfileBuffer);
    assertNotNull(result);
  ...
  public void testTransformBaseAndDerivedFilter() throws IOException {
    ...
    InputStream isAbstract =
        abstractClazz.getClassLoader().getResourceAsStream("com/amadeus/session/agent/MockAbstractFilter.class");
    classFileBuffer = readFully(isAbstract);
    byte[] abstractResult = sct.transform(abstractClazz.getClassLoader(), abstractClazz.getName(), abstractClazz,
        protectionDomain, classFileBuffer);
    assertNotNull(abstractResult);

    Class<MockDerivedFilter> derivedClazz = MockDerivedFilter.class;
    InputStream isDerived =
        derivedClazz.getClassLoader().getResourceAsStream("com/amadeus/session/agent/MockDerivedFilter.class");
    classFileBuffer = readFully(isDerived);
    byte[] derivedResult = sct.transform(derivedClazz.getClassLoader(), derivedClazz.getName(), derivedClazz,
        protectionDomain, classFileBuffer);
